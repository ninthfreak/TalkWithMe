...
Personas are the exception — they live in `Personas/`, not in YAML (see below).
For the rest, the same names appear twice and the difference matters:

| Where | What it is |
|-------|------------|
| `config/settings.yaml`, `config/chatrooms.yaml`, `config/player.yaml` | **Your** live config. The app reads and writes only these. Gitignored. |
