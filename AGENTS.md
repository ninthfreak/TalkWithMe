...
**Reply slots.** `_chat_stream()` tracks `attempted_personas` and `replied_personas` separately. A persona whose reply the guard cuts to nothing has been tried but has not replied, so it does not consume one of `max_persona_replies`; the loop moves to the next unattempted persona. Each persona is still tried at most once per turn, so the loop is bounded by the room's size. If nobody produces a usable reply the turn emits `NO_USABLE_REPLY_MESSAGE` before `complete` — a turn ending with a `start` event and nothing after it is indistinguishable from a hang.
