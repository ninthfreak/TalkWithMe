...
**Persona drafting.** `app/services/persona_draft.py` holds the substance: `LEVERS` (the eight things that actually change how a model plays a character) and `ANTI_PATTERNS` (the four things people write instead that do nothing). Both go into the drafting prompt, and `LEVERS` is *also* rendered into the persona-draft dialog by `app/main.py::index` — server-side on purpose, because a hand-copied list in the JS would drift away from what the model is actually told. `POST /api/personas/draft` sends the brief and nothing else — the existing cast is read only to keep the drafted name unique. An earlier version sent every persona to the model and asked for someone unlike them; that was the wrong mechanism (the prompt grew with the cast, and it defined a character by what the others were), and it is gone by request. Distinctness comes from the levers alone. Both endpoints call `chat_completion()` with `PROSE_TIMEOUT` (120s, matching the streaming path), not the 15s router default: a hundred-word draft on a local model hit that default every time, came back as `""`, and the server then finished generating into a closed connection. Output is parsed from labelled blocks (`NAME:`, `SYSTEM_PROMPT:` …) rather than JSON — local models follow labels far more reliably, and a malformed brace costs the entire draft where a missing label costs one field. `parse_draft()` clamps every field to the create form's limits, strips slashes from the name, and tolerates code fences, chatty preambles, lowercase labels, quoted values and invented enum members; nothing usable is a 503, never a blank form. `critique()` runs locally rather than asking the model, because the model is the wrong judge of exactly these failures. `POST /api/personas/preview` runs an unsaved draft against a question, optionally beside a real persona; it builds the **real** room preamble (a preview of a simpler prompt would preview something the app never runs) and puts both sides in a room of one, so the persona prompt is the only thing that differs. Neither endpoint writes to disk — the form is the review step, as with the suggested player message.
...
| `POST` | `/api/personas/draft` | Draft a persona from a brief |
