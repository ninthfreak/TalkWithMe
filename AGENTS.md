...
**Player profile.** The human's character lives in `config/player.yaml` (`PlayerConfig`), beside `personas.yaml` — that file holds the AI characters, this one holds the human. **It is not room data.** A room can *require* a character (`ChatRoom.require_player_profile`, a property of the room, in the room editor); the character itself belongs to the player and interacts with whichever room they are in. It was `ChatRoom.player_profile`; schema v5 drops the room key and `lift_player_profile()` carries one value into `player.yaml` first, so upgrading loses nothing — when several rooms had characters the first named one wins and the rest are named in the log. `appearance` is the "picture" and is deliberately text: the LLM is its only audience and it reads descriptions. `PlayerProfile.is_complete` (name **and** description; appearance optional) gates chat in rooms that require one — `_chat_stream()` emits `PROFILE_REQUIRED_MESSAGE` and stops **before** recording the user message, so nothing half-happens. `_user_label()` in `app/routers/chat.py` takes the name from here and is the single source for it: the preamble, the `[Name]: ` history tags, the stop strings and the reply guard must all agree.
...
| `GET` | `/api/player` | Get the player's own character |
| `PUT` | `/api/player` | Set the player's own character |
