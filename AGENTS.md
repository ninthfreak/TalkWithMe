...
| `chatrooms.js` | Chat room dropdown, room filtering, room editor, persona picker modal, room switching with history load |
...
**Suggested player message.** `POST /api/chat/suggest` drafts the player's *own* next message — the deliberate inverse of everything the reply guard enforces, and fine because the player asked for it and the draft lands in their input box to edit; nothing is sent or persisted until they press send. `_build_suggestion_prompt()` in `app/routers/chat.py` supplies the player's own last 8 messages as a voice sample (their words carry how they write better than any description), the windowed transcript for context, the room's persona list, and the character profile **written in the first person** — deliberately not `_player_lines()`, which is addressed to personas talking *to* the player and ends "never write their lines for them", the exact opposite of this task. It calls `chat_completion()` with the configured sampling temperature rather than the router's 0.1, and runs the draft through `ReplyGuard` pointed the other way (speaker = the player) so a `"[Tony]: "` prefix is stripped and a draft running on into a persona's reply is cut. An empty result is a 503, not a silently blank box.

**Echo.** `ChatRequest.echo` is a per-message flag, set by the Echo button beside the input box. When true the LLM is bypassed entirely: exactly one persona (picked per the normal selection mode) echoes the message verbatim as a single `token` event, `max_persona_replies` is forced to 1 for that turn, and no preamble, length tier, stop list or reply guard applies — nothing was generated. **It is deliberately not a room setting.** It used to be `ChatRoom.echo_chamber` in `chatrooms.yaml`, which turned a one-off act ("make this persona say this line") into a mode the room stayed in until someone switched it off. Schema v3 drops the stored key with a log line. Do not reintroduce it as room state.
...
**Frontend room settings** all write through `saveRoomSettings()` in `static/chatrooms.js`, which sends the partial update and merges the response into the cached `allChatRooms` entry. The room editor dialog (`#room-edit-overlay`, opened from Edit in the chat rooms list) is the canonical place to configure a room; the left-panel echo-chamber and require-character toggles are shortcuts for the selected room and go through the same helper, so the two cannot disagree.
