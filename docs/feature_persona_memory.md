...
- Every memory names WHO it is about: the `about` argument is the name the transcript tags them with, and the stored line is prefixed `[Name]`. The human is filed under the persona they are playing, so playing somebody else makes you somebody else to the room.
