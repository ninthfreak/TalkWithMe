...
- Every memory names WHO it is about: the `about` argument is the name the transcript tags them with, and the stored line is prefixed `[Name]`. The human is filed under the persona they are playing, so playing somebody else makes you somebody else to the room. `User` is the name for the human playing as themselves and for nobody else: while a persona is adopted, nothing filed under `User` is read and nothing new is filed there, and untagged legacy lines — which were all about the human as themselves — are withheld too. A model that names the human its own way ("the user") has the subject normalised to the name this room uses, in either direction.
- Write the memory in your own voice about them by name ("Tony has never been on a boat"), not as a report to a third party ("The user told me...")
- Save what any of the people present revealed about themselves, filed under them. Never save anything about yourself: a memory of what somebody else is like, stored unattributed, comes back in every later conversation as though it were true of the persona holding it — which is how a whole cast ends up sharing one personality.
- Because a memory is filed under a name, renaming somebody has to move their memories with them. That is what `POST /api/personas/{name}/rename` is for, and why `PUT` refuses a changed name: the `[Subject]` tag, the `met.txt` entry and the transcript tag are the same string, and a save that changed one of them would orphan the rest. The subject tag is always repointed; the old name *inside* a memory's text is swept behind a flag (default on), because a name like Will or May is also an ordinary word.
- A memory records whether it was **witnessed or worked out**: `[Tony] (assumed) Tony is about forty.` Injection says which is which, so a persona can be wrong out loud rather than treating a guess as something it was told, and assumptions are shed before facts when the byte budget fills.
- The cast a persona may write about is **everyone in the room**, not just whoever spoke — with one reply per message by default, the latter collapsed to the human's character and personas never remembered each other. The per-pass cap is shared round-robin so one person cannot take every slot.
- The reflection is shown **what the persona already knows** about the people present and asked only for what is new. That, not the prompt wording, is why the same fact stops being filed every conversation — the question used to be asked in ignorance each time.
- Exact duplicates are trimmed automatically by a plain function (byte-identical only, so it cannot lose a real memory). Merging notes that merely *say* the same thing needs a model and is hand-run — "Condense memories…" in the persona editor, which previews before it writes and can also drop a guess that a later fact has settled. A rewrite that would lose a number or a name from what the persona was *told* about somebody is refused for that person and the preview names the detail.
- Memories are written by two routes into the same file. The `add_memory` **tool** requires `allow_tool_calls`, which is off by default and whose label is about MCP servers — and turning it on moves that persona off the transcript prompt format, because the tool-calling path builds a chat-completions payload. The **reflection pass** (`app/services/reflection.py`) does not: when a conversation ends, each persona who spoke is asked once, in a plain completion, what it learned about the people in it, and the answer is filed the same way. Needing the model to make the judgement is not the same as needing a tool to deliver it.
- A companion file, `met.txt`, records who a persona has shared a room with. The app writes it, not the model, so "you have never met" is a fact rather than an inference from an empty memories file — and so a first meeting reads as one even when memories are switched off.
