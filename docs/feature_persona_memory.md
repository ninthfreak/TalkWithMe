...
- Write the memory in your own voice about them by name ("Wes has never been on a boat"), not as a report to a third party ("The user told me...")
...
- A memory records whether it was **witnessed or worked out**: `[Wes] (assumed) Wes is about forty.` Injection says which is which, so a persona can be wrong out loud rather than treating a guess as something it was told, and assumptions are shed before facts when the byte budget fills.
